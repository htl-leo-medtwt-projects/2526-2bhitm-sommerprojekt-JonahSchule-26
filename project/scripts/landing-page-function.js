...
function showLeaderboardScreen() {
  ...
  document.getElementById("first-place").innerHTML = first
    ? `${first.name}<br>${formatTime(first.time)}<br>${first.date}`
    : "—";
  document.getElementById("second-place").innerHTML = second
    ? `${second.name}<br>${formatTime(second.time)}<br>${second.date}`
    : "—";
  document.getElementById("third-place").innerHTML = third
    ? `${third.name}<br>${formatTime(third.time)}<br>${third.date}`
    : "—";
}
  ...
  return {
    musicVolume:
      savedMusicVolume !== null
        ? parseInt(savedMusicVolume)
        : defaultOptions.musicVolume,
    sfxVolume:
      savedSfxVolume !== null
        ? parseInt(savedSfxVolume)
        : defaultOptions.sfxVolume,
  };
}
...
function endTutorial() {
  ...
  if (title && text) {
    ...
    text.textContent =
      "You're now ready to race! Click the button below to start the game.";
